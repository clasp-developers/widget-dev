...
(defclass comm ()
  ((kernel :initarg :kernel :accessor kernel :initform *kernel*)
   (comm-id :initarg :comm-id :accessor comm-id :initform (uuid:make-v4-uuid))
   (primary :initarg :primary :accessor primary :initform t :type boolean :documentation “Am I the primary or secondary Comm?”)
...
(defmethod open ((self comm) &key (data (%open-data self)) metadata buffers)
  “Open the frontend-side version of this comm”
   (let ((comm-manager (or (comm-manager (kernel self)) (error “Comms cannot be opened without a kernel and a comm-manager attached to that kernel.”))))
...
(defmethod close ((self comm) &key data metadata buffers)
  ...
  (unregister-comm (comm-manager (kernel self)) self))
